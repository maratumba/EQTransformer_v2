...
def predictor(input_dir=None,
    ...
    """
    Applies a trained model to a windowed waveforms to perform both detection and picking at the same time. 
    ...
    ----------
       ...
       input_hdf5: str, (default = None)
           Path to a hdf5 file containing only one class of "data" with numpy arries containing 3 component waveforms each 1 min long.
       ...
       output_probabilities: bool, (default = False)
           If True, it will output probabilities and estimated uncertainties for each trace into a hdf file.
       ...
       number_of_sampling: positive integer, (default = 5)
           Number of Monte Carlo sampling to estimate the uncertainties. 
       ...
       number_of_cpus: positive integer, (default = 5) 
           Number of CPUs used for the parrallel preprocessing and feeding of data for prediction.   
       ...
       spLimit : int, default : 60
           S - P time in seconds. It will limit the results to those detections with events that have specific S-P time limit.
           
    rubric:: Notes
        Estimating the uncertainties requiers multiple predictions and will increase the computational time. 

    Generates
    -------
    ./output_dir/X_prediction_results.csv
        A table containing all the detection, and picking results. Douplicated events are already removed.
        
    ./output_dir/X_report.txt
        A summary of parameters used for the prediction and perfomance.
        
    ./output_dir/figures 
        A folder containing plots. 

    ./output_dir/output_probabilities.hdf5 
        An hdf5 containing the predicted probabilities for all samples.  
    ...
    parser.add_argument("--output_probabilities", default=output_probabilities)    
    ...
    parser.add_argument("--estimate_uncertainty", default=estimate_uncertainty)
    parser.add_argument("--number_of_sampling", default=number_of_sampling)
    ...
    parser.add_argument("--number_of_cpus", default=number_of_cpus) 
    parser.add_argument("--use_multiprocessing", default=use_multiprocessing) 
    parser.add_argument("--keepPS", default=keepPS)  
    ...
    for ct, st in enumerate(station_list):
        ...
        with open(os.path.join(save_dir,'X_report.txt'), 'a') as the_file:    
            ...
            the_file.write('plot_mode: '+str(args.plot_mode)+'\n')                                                
...
def _gen_predictor(new_list, args, model): 
    ...
    """ 
    Peforms the predictions for the current batch.
    ...
    model: 
        Compiled model used for the prediction.

    Returns
    -------
    prob_dic : dic
        A dictionary containing output probabilites and their estimated standard deviations.
    """    
...
def _gen_writer(new_list, args, prob_dic, pred_set, HDF_PROB, predict_writer, save_figs, csvPr_gen, plt_n, detection_memory, keepPS, spLimit):
    ...
    prob_dic : dic
        A dictionary containing output probabilites and their estimated standard deviations.
    pred_set : dic
        A dictionary containing hdf datesets for the current batch. 
    HDF_PROB : obj
        For writing out the probabilites and uncertainties. 
    predict_writer : obj
    ...
    """    
    det_buffer = len(detection_memory)
