...
from keras.layers import add, Activation, LSTM, Conv1D, Conv2D, SpatialDropout2D, Reshape
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        X, Xtf, y1, y2, y3 = self.__data_generation(list_IDs_temp)
        return ({'input1': X, 'input2': Xtf}, {'detector': y1, 'picker_P': y2, 'picker_S': y3})
    ...
    def drop_channel(self, data, snr, rate):
        data = np.copy(data)
    ...
    def drop_channel_noise(self, data, rate):
        data = np.copy(data)
    ...
    def add_noise(self, data, snr, rate):
        data_noisy = np.empty((data.shape))
        if np.random.uniform(0, 1) < rate and all(snr >= 10.0): 
            data_noisy = np.empty((data.shape))
            data_noisy[:, 0] = data[:,0] + np.random.normal(0, np.random.uniform(0.01, 0.11)*max(data[:,0]), data.shape[0])
            data_noisy[:, 1] = data[:,1] + np.random.normal(0, np.random.uniform(0.01, 0.11)*max(data[:,1]), data.shape[0])
            data_noisy[:, 2] = data[:,2] + np.random.normal(0, np.random.uniform(0.01, 0.11)*max(data[:,2]), data.shape[0])    
        else:
            data_noisy = data
        return data_noisy           
    ...
    def add_event(self, data, addp, adds, coda_end, snr, rate): 
        added = np.copy(data)
    ...
    def shift_event(self, data, addp, adds, coda_end, snr, rate): 
        org_len = len(data)
        data2 = np.copy(data)
    ...
    def PreEmphasis(self, data, pre_emphasis = 0.97):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        Xtf = np.empty((self.batch_size, 151, 41, self.n_channels))
            ...
            if self.augmentation == True:                 
                if i <= self.batch_size//2:   
                    if self.shift_event_r and dataset.attrs['trace_category'] == 'earthquake_local':
                        data, spt, sst, coda_end = self.shift_event(data, spt, sst, coda_end, snr, self.shift_event_r/2); 
                    ...
                    if dataset.attrs['trace_category'] == 'earthquake_local':                   
                        if self.shift_event_r:
                            data, spt, sst, coda_end = self.shift_event(data, spt, sst, coda_end, snr, self.shift_event_r); 
                            
                        if self.add_event_r:
                            data, additions = self.add_event(data, spt, sst, coda_end, snr, self.add_event_r); 
                                
                        if self.add_noise_r:
                            data = self.add_noise(data, snr, self.add_noise_r);
    
                        if self.drop_channe_r:    
                            data = self.drop_channel(data, snr, self.drop_channe_r);
                            data = self.adjust_amplitude_for_multichannels(data)  
                                    
                        if self.scale_amplitude_r:
                            data = self.scale_amplitude(data, self.scale_amplitude_r); 
                                    
                        if self.pre_emphasis:  
                            data = self.PreEmphasis(data) 
                        ...
                        if self.drop_channe_r:    
                            data = self.drop_channel_noise(data, self.drop_channe_r);
                            
                        if self.add_gap_r:    
                            data = self.add_gaps(data, self.add_gap_r)
            ...
            if dataset.attrs['trace_category'] == 'earthquake_local':  
                ...
                if self.label_type  == 'gaussian': 
                    ...
                    if spt and (spt-20 >= 0) and (spt+21 < self.dim):
                        y2[i, spt-20:spt+21, 0] = self.label()
                    elif spt and (spt+21 < self.dim):
                        y2[i, 0:spt+spt+1, 0] = self.label(a=0, b=spt, c=2*spt)
                    elif spt and (spt-20 >= 0):
                        pdif = self.dim - spt
                        y2[i, spt-pdif-1:self.dim, 0] = self.label(a=spt-pdif, b=spt, c=2*pdif)
         
                    if sst and (sst-20 >= 0) and (sst+21 < self.dim):
                        y3[i, sst-20:sst+21, 0] = self.label()
                    elif sst and (sst+21 < self.dim):
                        y3[i, 0:sst+sst+1, 0] = self.label(a=0, b=sst, c=2*sst)
                    elif sst and (sst-20 >= 0):
                        sdif = self.dim - sst
                        y3[i, sst-sdif-1:self.dim, 0] = self.label(a=sst-sdif, b=sst, c=2*sdif)    
        ...
        return X.astype('float32'), Xtf.astype('float32'), y1.astype('float32'), y2.astype('float32'), y3.astype('float32')
...
def data_reader( list_IDs, 
    ...
    def _pre_emphasis(data, n_channels, pre_emphasis = 0.97):
    ...
    if augmentation == True:
        ...
        Xtf = np.zeros((2*len(list_IDs), 151, 41, n_channels))        
        ...
        Xtf = np.zeros((len(list_IDs), 151, 41, n_channels))        
    ...
    for i, ID in enumerate(list_IDs):
        ...
        if augmentation == True:                 
            ...
            X[i, :, :] = data1 
            if not np.any(np.isnan(data1).any()):
                for ch in range(3): 
                    bpf = data1[:, ch]                        
                    f, t, Pxx = signal.stft(bpf, fs = 100, nperseg=80)
                    Pxx = np.abs(Pxx)
            ...
            if not np.any(np.isnan(data2).any()):
                for ch in range(3): 
                    bpf = data2[:, ch]                        
                    f, t, Pxx = signal.stft(bpf, fs = 100, nperseg=80)
                    Pxx = np.abs(Pxx)
                    Xtf[len(list_IDs)+i, :, :, ch] = Pxx.T                    
...
class PreLoadGeneratorTest(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        X, Xtf = self.__data_generation(list_IDs_temp)
        return ({'input1': X, 'input2': Xtf})
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        return X.astype('float32'), Xtf.astype('float32')
...
def picker(args, yh1, yh2, yh3, yh1_std, yh2_std, yh3_std, spt=None, sst=None):
 ...
 #   pp_arr = trigger_onset(yh2, args.P_threshold, args.P_threshold)
 #   ss_arr = trigger_onset(yh3, args.S_threshold, args.S_threshold) 
...
def block_BiLSTM(filters, drop_rate, padding, inpR):
    """
...
def block_CNN_1(filters, ker, drop_rate, activation, padding, inpC): 
    """
...
def block_CNN_2(filters, ker, drop_rate, activation, padding, inpC): 
    """
    Returns CNN residual blocks
    """
    prev = inpC
    ...
    act_1 = Activation(activation)(layer_1)
    act_1 = SpatialDropout2D(drop_rate)(act_1, training=True)
    conv_1 = Conv2D(filters, (ker-2, ker-2), padding = padding)(act_1) 
    
    layer_2 = BatchNormalization()(conv_1) 
    act_2 = Activation(activation)(layer_2) 
    act_2 = SpatialDropout2D(drop_rate)(act_2, training=True)  
    conv_2 = Conv2D(filters, (ker-2, ker-2), padding = padding)(act_2) 
...
def encoder(filter_number, filter_size, depth, drop_rate, ker_regul, bias_regul, activation, padding, inpC):
    """
...
def decoder(filter_number, filter_size, depth, drop_rate, ker_regul, bias_regul, activation, padding, inpC):
    """
...
class cred2:
    ...
    Args:
        ...
        endcoder_depth: number of layers in the encoder.
        decoder_depth: number of layers in the decoder.
        rnn_depth: number of blocks in residual BiLTSM.            
    Returns:
        The complied model.
    """
    ...
    def __call__(self, inp1, inp2):
        ...
        conv2D_2 = Conv2D(self.nb_filters[0], (11,11), strides=(2, 2), padding = self.padding, activation = self.activationf)(inp2) 
        conv2D_2 = Conv2D(self.nb_filters[0], (11,11), strides=(2, 2), padding = self.padding)(inp2)     
        conv2D_3 = Conv2D(self.nb_filters[1], (7,7), strides = (3, 3), padding = self.padding, activation = self.activationf)(conv2D_2)
        conv2D_4 = Conv2D(self.nb_filters[1], (5,5), strides = (2, 2), padding = self.padding, activation = self.activationf)(conv2D_3)
        res_conv_4 = block_CNN_2(self.nb_filters[1], 5, self.drop_rate, self.activationf, self.padding, conv2D_4)      
        shape = K.int_shape(res_conv_4)
        shaped = Reshape((shape[1]*shape[2], shape[3]))(res_conv_4)      
        reshaped = Cropping1D(cropping=(4,1))(shaped)           

        
        x = inp1
        x = encoder(self.nb_filters, 
                    self.kernel_size, 
                    self.endcoder_depth, 
                    self.drop_rate, 
        ...
        for cb in range(self.cnn_blocks):
            x = block_CNN_1(self.nb_filters[6], 3, self.drop_rate, self.activationf, self.padding, x)
            if cb > 2:
                x = block_CNN_1(self.nb_filters[6], 2, self.drop_rate, self.activationf, self.padding, x)
        x = keras.layers.concatenate([reshaped, x])
        for bb in range(self.BiLSTM_blocks):
            x = block_BiLSTM(self.nb_filters[1], self.drop_rate, self.padding, x)
        
       # x = keras.layers.concatenate([reshaped, x])

        x, weightdD0 = transformer(self.drop_rate, None, 'attentionD0', x)             
        encoded, weightdD = transformer(self.drop_rate, None, 'attentionD', x)             
            
        decoder_D = decoder([i for i in reversed(self.nb_filters)], 
        ...
        decoder_P = decoder([i for i in reversed(self.nb_filters)], 
                            [i for i in reversed(self.kernel_size)], 
                            self.decoder_depth, 
                            self.drop_rate, 
                            self.kernel_regularizer, 
        ...
        decoder_S = decoder([i for i in reversed(self.nb_filters)], 
                            [i for i in reversed(self.kernel_size)],
                            self.decoder_depth, 
                            self.drop_rate, 
                            self.kernel_regularizer, 
        ...
        if self.multi_gpu == True:
            parallel_model = Model(inputs=[inp1, inp2], outputs=[d, P, S])
            model = multi_gpu_model(parallel_model, gpus=self.gpu_number)
        else:
            model = Model(inputs=[inp1, inp2], outputs=[d, P, S])
